...
function AuthScreen({ onLogin }: { onLogin: (user: User | DemoUser, demo?: boolean) => void }) {
  ...
  async function doLogin() {
    ...
    try { const c = await signInWithEmailAndPassword(auth, email, pw); onLogin(c.user); }
  ...
  async function doRegister() {
    ...
    try {
      ...
      await setDoc(doc(db, 'users', cred.user.uid), { displayName: name, email: regEmail, createdAt: serverTimestamp() });
      onLogin(cred.user);
...
export default function MemberPage() {
  ...
  useEffect(() => {
    ...
    const unsub = onAuthStateChanged(auth, async user => {
      if (user) {
        setCurrentUser(user);
        try {
          const fontsSnap = await getDocs(collection(db, 'users', user.uid, 'fonts'));
  ...
  async function handleLogout() {
    ...
    await signOut(auth); setCurrentUser(null); setState('auth');
  }
